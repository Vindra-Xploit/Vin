...
""")#line:49
    198626 =input (f"{H}[{P}?{H}]{P} ===> Pilih :{K} ")#line:50
    if 198626 in ['1','01']:#line:51
      print (f"{H}[{P}!{H}]{P} Anda Akan Diarahkan Ke Whatsapp...");time .sleep (3 );os .system ('xdg-open https://wa.me/6283866965786?text=Bang+Beli+Lisensi+nya+dong.......???????');exit ()#line:52
    elif 198626 in ['2','02']:#line:53
      198626 =input (f"{H}[{P}?{H}]{P} Api Key :{K} ")#line:54
      if len (198626 )==0 :#line:55
        exit (f"{P}[{M}!{P}]{M} Jangan Kosong Tolol")#line:56
      else :#line:57
        with requests .Session ()as 198626 :#line:58   #### ISI TOKEN LU DAN   ID LU
          198626 =198626 .get(f'https://app.cryptolens.io/api/key/activate?token=WyIxNjk4MzkyOCIsIktFWUhHaUJzQkZzcEpTdXFRMXh0ZUh3U0crOWpyNk9LM1ZWV0xSQlkiXQ==&ProductId=14877&Key={198626}&Sign=True').json ()['licenseKey']#line:59
          open ('apikey.txt','w').write (198626 )#line:60
          print (f"{H}[{P}*{H}]{P} Expired :{K} {198626['expires'].split('T')[0]}");time .sleep (2 );login()#line:61
    elif 198626 in ['3','03']:#line:62
      exit ()#line:63
    else :#line:64
      exit (f"{P}[{M}!{P}]{M} Wrong Input")#line:65
  except (KeyError ):#line:66
    exit (f"{P}[{M}!{P}]{M} Lisensi Invalid")#line:67
  except Exception as 198626 :#line:68
    exit (f"{P}[{M}!{P}]{M} {198626}")#line:69
